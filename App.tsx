...
const App = () => {
  ...
  const generatePasswordString = (passwordLength: number) => {
    ...
    if (upperCase) charecterList += uppercaseChars;
    if (lowerCase) charecterList += lowercaseChars;
    if (numbers) charecterList += digitChars;
    if (symbols) charecterList += specialChars;
  ...
  const createPassword = (charecters: string, passwordLength: number) => {
    ...
    for (let i = 0; i < passwordLength; i++) {
      result += charecters.charAt(
        Math.round(Math.random() * charecters.length),
      );
    }
    return result;
  };
